...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    SET_JOBS(state, newJobs) {
      state.jobs = newJobs;
    },
    SET_JOBS_LOCAL(state, newJobs) {
      state.jobs_local = newJobs;
    },
  ...
  actions: {
    ...
    async fetchJobs({ commit }) {
      ...
      .onSnapshot((snapshot) => {
        ...
        snapshot.forEach((doc) => {
          let job = doc.data();
          job.id = doc.id;
          job.pubDate = Date.parse(job.pubDate);
          jobsArray.push(job);
        });
      ...
      .onSnapshot((snapshot) => {
        ...
        commit("SET_JOBS_LOCAL", jobsArray);
        // return jobsArray;
      });
    },
    ...
    async deletePost({ dispatch }, idToRemove) {
      ...
      return await fb.postsCollection
        .doc(idToRemove)
        .delete()
        .then(() => {
